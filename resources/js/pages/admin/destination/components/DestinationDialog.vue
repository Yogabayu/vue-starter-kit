...
const selectedCategories = computed(() => {
  ...
  return selectedCategoryIds.value.map((id:any) => map.get(id)).filter(Boolean)
})
...
watch(() => props.uploadCaption, v => (localCaption.value = v))
                  ...
                  <Command>
                    ...
                    <CommandGroup>
                      <CommandItem
                        ...
                        @select="emit('toggle-category', c.id)"
                        :value="c.id"
                      >
                        <span>{{ c.name }}</span>
                        <Check
                          v-if="selectedCategoryIds.includes(c.id)"
                          class="h-4 w-4"
                        />
                      </CommandItem>
                    </CommandGroup>
          ...
          <FormItem class="mt-4">
            ...
            <FormControl>
              <div class="flex flex-wrap items-center gap-2">
                ...
                <Button variant="outline" size="sm" :disabled="props.isUploading" @click="fileEl?.click()">
                  Choose File
                </Button>
          ...
          <div class="grid gap-3">
            ...
            >
              <div
                ...
                :key="img.kind === 'pending' ? 'p-' + img.uid : 'i-' + img.id"
                class="group relative overflow-hidden rounded-lg border shadow-sm"
              >
                <img
                  :src="img.kind === 'pending' ? img.preview_url : img.image_url"
                ...
                >
                  <span>
                    <template v-if="img.is_cover">Cover</template>
                    <template v-else>{{ img.caption || '\u00A0' }}</template>
                  </span>
                  <div class="flex items-center gap-2">
                    <button
                      v-if="!img.is_cover"
                      @click.prevent="emit('set-cover', img)"
                      class="underline hover:opacity-80"
                    >
                      Set cover
                    </button>
                    <button
                      @click.prevent="emit('delete-image', img)"
                      class="hover:text-destructive"
                      title="Delete image"
                    >
